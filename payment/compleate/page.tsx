...
const STATUS_CONTENT_MAP: Record<
  PaymentStatus,
  { text: string; iconColor: string; icon: React.JSX.Element }
> = {
  succeeded: {
...
const CompletePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    stripe.retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
      ...
      setStatus(paymentIntent.status as PaymentStatus); // Explicitly cast the status to PaymentStatus
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <h2 id="status-text" className="mt-4 text-xl font-semibold text-center">
        {STATUS_CONTENT_MAP[status].text}
      </h2>
      {intentId && (
        <div id="details-table" className="mt-6">
          <table className="w-full table-auto border-collapse">
            <tbody>
              <tr>
                <td className="font-medium text-gray-600 p-2">id</td>
                <td className="text-gray-800 p-2">{intentId}</td>
              </tr>
              <tr>
                <td className="font-medium text-gray-600 p-2">status</td>
                <td className="text-gray-800 p-2">{status}</td>
              </tr>
            </tbody>
          </table>
        </div>
